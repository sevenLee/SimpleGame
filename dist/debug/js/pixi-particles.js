 ...
 */
(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.pixiParticles = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof _dereq_=="function"&&_dereq_;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof _dereq_=="function"&&_dereq_;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(_dereq_,module,exports){
	"use strict";
	 ...
	 *
	 *     {
	 *         //framerate is _dereq_d. It is the animation speed of the particle in frames per
	 *         //second.
	 ...
	 *         loop: true,
	 *         //textures is _dereq_d, and can be an array of any (non-zero) length.
	 *         textures: [
	...
	var Emitter = function(particleParent, particleImages, config)
	{
		...
		this.startAlpha = 1;
		/**
		 * The starting alpha variation of all particles.
		...
		/**
		 * The starting/ending speed of all particles.
		 * @property {Number} speed
		 * @default 0
		 */
		this.speed = 0;
		/**
		 * The speed variance of all particles.
		...
		this.maxSpeed = NaN;
		/**
		 * The starting size of all particles.
		 * @property {Number} startSize
		 * @default 1
		 */
		this.startSize = 1;
		/**
		 * The starting size variance of all particles.
		 * @property {Number} startSizeVariance
		 * @default 0
		 */
		this.startSizeVariance = 0;
		/**
		 * The ending size of all particles.
		 * @property {Number} endSize
		 * @default 1
		 */
		this.endSize = 1;
		/**
		 * The ending size variance of all particles.
		 * @property {Number} endSizeVariance
		 * @default 0
		 */
		...
		this.endColor = null;
		/**
		 * The lifetime for a particle, in seconds.
		 * @property {Number} lifetime
		 */
		this.lifetime = 0;
		/**
		 * The lifetime variance for a particle, in seconds.
		 * @property {Number} lifetimeVariance
		 */
		this.lifetimeVariance = 0;
		/**
		 * The start rotation for a particle, in degrees. This value
		 * is ignored if the spawn type is "burst" or "arc".
		 * @property {Number} startAngle
		 */
		this.startAngle = 0;
		/**
		 * The rotation variance variance for a particle, in degrees. This value
		 * is ignored if the spawn type is "burst" or "arc".
		 * @property {Number} startAngleVariance
		 */
		this.startAngleVariance = 0;
		/**
		 * The start rotation for a particle, in degrees.
		 * @property {Number} rotationStart
		 */
		this.rotationStart = 0;
		/**
		 * The start rotation variance for a particle, in degrees.
		 * @property {Number} rotationStartVariance
		 */
		...
		/**
		 * The start rotation variance for a particle, in degrees.
		 * @property {Number} rotationEndVariance
		 */
		 ...
		 * Setting this to false _dereq_s calling the update function manually.
	...
	*/
	...
	p.init = function(art, config)
	{
		...
		///////////////////////////
		//set up the alpha
		this.startAlpha = parseFloat(config.startColorAlpha);
		this.startAlphaVariance = parseFloat(config.startColorVarianceAlpha);
		this.endAlpha = parseFloat(config.finishColorAlpha);
		this.endAlphaVariance = parseFloat(config.finishColorVarianceAlpha);
		//set up the speed
		this.speed = parseFloat(config.speed);
		this.speedVariance = parseFloat(config.speedVariance);
		//set up acceleration
		this.acceleration = new PIXI.Point( parseFloat(config.gravityx), parseFloat(config.gravityy) );
		//set up the size
		this.startSize = parseFloat(config.startParticleSize);
		this.startSizeVariance = parseFloat(config.startParticleSizeVariance);
		this.endSize = parseFloat(config.finishParticleSize);
		this.endSizeVariance = parseFloat(config.finishParticleSizeVariance);
		//set up the color
		this.startColor = [];
		this.startColor.push( (parseFloat(config.startColorRed) + parseFloat(config.startColorVarianceRed) * ( Math.random() * 2.0 - 1.0 )) * 255 );
		this.startColor.push( (parseFloat(config.startColorGreen) + parseFloat(config.startColorVarianceGreen) * ( Math.random() * 2.0 - 1.0 )) * 255 );
		this.startColor.push( (parseFloat(config.startColorBlue) + parseFloat(config.startColorVarianceBlue) * ( Math.random() * 2.0 - 1.0 )) * 255 );
		this.endColor = [];
		this.endColor.push( (parseFloat(config.finishColorRed) + parseFloat(config.finishColorVarianceRed) * ( Math.random() * 2.0 - 1.0 )) * 255 );
		this.endColor.push( (parseFloat(config.finishColorGreen) + parseFloat(config.finishColorVarianceGreen) * ( Math.random() * 2.0 - 1.0 )) * 255 );
		this.endColor.push( (parseFloat(config.finishColorBlue) + parseFloat(config.finishColorVarianceBlue) * ( Math.random() * 2.0 - 1.0 )) * 255 );
		//set up the start rotation
		this.startAngle = parseFloat(config.angle);
		this.startAngleVariance = parseFloat(config.angleVariance);
		//set up the rotation speed
		this.rotationStart = parseFloat(config.rotationStart);
		this.rotationStartVariance = parseFloat(config.rotationStartVariance);
		this.rotationEnd = parseFloat(config.rotationEnd);
		this.rotationEndVariance = parseFloat(config.rotationEndVariance);
		//set up the lifetime
		this.lifetime = parseFloat(config.particleLifespan);
		//for some weird config we must prevent this field to be 0 ( in ParticleDesigner min is 0.05 )
		if ( this.lifetime == 0 ) {
			this.lifetime = 0.05;
		}
		this.lifetimeVariance = parseFloat(config.particleLifespanVariance);
		//get the blend mode
		...
		// 	case "rect":
				...
				this.spawnRect = new PIXI.Rectangle(-parseFloat(config.sourcePositionVariancex), -parseFloat(config.sourcePositionVariancey), 
					parseFloat(config.sourcePositionVariancex) * 2.0, parseFloat(config.sourcePositionVariancey) * 2.0);
		// 		break;
		// 	case "circle":
		...
		this.emitterLifetime = parseFloat(config.duration);
		//set the max particles
		this.maxParticles = parseInt(config.maxParticles) > 0 ? parseInt(config.maxParticles) : 1000;
		//set the spawning frequency
		// this.frequency = config.frequency;
		this.frequency = this.lifetime / this.maxParticles; // lock for now
	...
	p.update = function(delta)
	{
		...
		if (this._emit)
		{
			...
			while(this._spawnTimer <= 0)
			{
				...
				var lifetime = this.lifetime + this.lifetimeVariance * ( Math.random() * 2.0 - 1.0 );
				...
				if(-this._spawnTimer < lifetime)
				{
					...
					for(var len = Math.min(this.particlesPerWave, this.maxParticles - this.particleCount); i < len; ++i)
					{
						...
						p.startAlpha = this.startAlpha + this.startAlphaVariance * (Math.random() * 2.0 - 1.0);
						p.endAlpha = this.endAlpha + this.endAlphaVariance * (Math.random() * 2.0 - 1.0);
						//speed is constant for now
						p.maxSpeed = p.startSpeed = p.endSpeed = this.speed + this.speedVariance * (Math.random() * 2.0 - 1.0);
						
						p.acceleration.x = this.acceleration.x;
						p.acceleration.y = this.acceleration.y;
						
						p.startScale = (this.startSize + this.startSizeVariance * (Math.random() * 2.0 - 1.0)) / p.texture.width;
						p.endScale = (this.endSize + this.endSizeVariance * (Math.random() * 2.0 - 1.0)) / p.texture.width;
						...
						p.rotationSpeed = ( this.rotationEnd + this.rotationEndVariance * ( Math.random() * 2.0 - 1.0 ) 
												- this.rotationStart - this.rotationStartVariance * ( Math.random() * 2.0 - 1.0 ) ) / lifetime;
	...
	p._spawnPoint = function(p, emitPosX, emitPosY)
	{
		...
		p.rotation = this.startAngle + this.startAngleVariance * (Math.random() * 2.0 - 1.0);
	...
	PathParticle.parseData = function(extraData)
	{
		...
		{
			if(ParticleUtils.verbose)
				console.error("PathParticle _dereq_s a path string in extraData!");
			output.path = null;
		}
		return output;
	};
	...
	{
		// Attempt to _dereq_ the pixi module
	...
	{
		throw "pixi-particles _dereq_s pixi.js to be loaded first";
	}
